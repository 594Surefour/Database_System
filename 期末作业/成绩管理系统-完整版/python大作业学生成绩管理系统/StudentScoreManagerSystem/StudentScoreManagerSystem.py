...
class PyMySQLUtils:
    ...
    def __init__(self):
        # 打开数据库连接
        self.db = pymysql.connect("localhost", "root", "root", "school")
        # 使用cursor()方法获取操作游标
...
class TeacherMenu:
    def __init__(self, parent_window):
        ...
        Entry(self.frame_left, textvariable=self.var_classmate_score, font=("宋体", 10)).grid(row=7, column=1, padx=5,
                                                                                            pady=5)
        Entry(self.frame_left, textvariable=self.var_teacher_score, font=("宋体", 10)).grid(row=8, column=1, padx=5,
                                                                                          pady=5)
        ...
        Button(self.frame_right, text="写入到Excel文件", width=30,
               command=lambda: mysql_excel("E:\\PycharmProjects\\StudentScoreManagerSystem\\NewStudentScore.xls")).grid(
        ...
        if messagebox.askyesnocancel("askyesnocancel", "是否修改该学生成绩信息？"):
            ...
            if sid not in self.list_sid:
                ...
                self.tree.item(self.tree.get_children()[sid_index], values=(
                    sid, sname, sdept, chinese, math, english, average_score, classmate_score, teacher_score,
                    total_score))
...
class prepare:
    ...
    def excel_mysql(self):
        book = xlrd.open_workbook("C:\\Users\\阮相歌\\Desktop\\python大作业学生成绩管理系统\\StudentScoreManagerSystem\\StudentScore.xlsx")
