...
def Buy_Ticket():
    ...
    def SearchTicket():
        def show_ticket():
            def CallOn_ofSearch():  
                root_confirm_buy=tk.Toplevel(window)    
                tk.Label(root_confirm_buy,text='你的选择是'+str(lb_show_ticket.get(lb_show_ticket.curselection()))+'行程').pack()
                purchased_ticket=lb_show_ticket.get(lb_show_ticket.curselection())
                global purchased_ticket_id
                purchased_ticket_id=purchased_ticket[0] 
                print(purchased_ticket_id)
                tk.Button(root_confirm_buy,text='确定',command=Purchased).pack()
                tk.Button(root_confirm_buy,text='取消',command=root_confirm_buy.destroy).pack()
            
            start = var_start_station.get()
            terminal = var_terminal_station.get()
            
            root_show_ticket=tk.Toplevel(window)
            root_show_ticket.geometry('500x500')
            record=session.query(tickets_info.ticket_id,tickets_info.start_station,
                                 tickets_info.terminal_station,tickets_info.stopping_station,
                                 tickets_info.ticket_price,tickets_info.available_seats,
                                 tickets_info.go_time,tickets_info.arr_time
                                 ).filter(tickets_info.start_station==start,tickets_info.terminal_station==terminal).all()
            lb_show_ticket = tk.Listbox(root_show_ticket,width=50) 
            tk.Label(root_show_ticket,text='检索结果如下：').pack()
            tk.Label(root_show_ticket,justify="left",text='始发站 终点站 经停 票价 剩余座位 出发时间 到达时间').pack()
            for item in record:
                lb_show_ticket.insert('end', item)  # 从最后一个位置开始加入值
            lb_show_ticket.pack()  
            
            confirm_buy=tk.Button(root_show_ticket,text='购买当前选中客票',command=CallOn_ofSearch)
            confirm_buy.place(x=300,y=300)
            
        
        
            
        window_search_ticket=tk.Toplevel(window)
        window_search_ticket.geometry('400x300')
        window_search_ticket.title('检索客票')
        
        tk.Label(window_search_ticket, text='输入出发地').place(x=50, y=10)  # 放置线路编号输入框
        entry_start_station = tk.Entry(window_search_ticket, textvariable=var_start_station) #创建entry
        entry_start_station.place(x=130, y=10)
        
        tk.Label(window_search_ticket, text='输入到达地').place(x=50, y=40)  # 放置线路编号输入框
        entry_terminal_station = tk.Entry(window_search_ticket, textvariable=var_terminal_station) #创建entry
        entry_terminal_station.place(x=130, y=40)
        
        confirm_search_ticket=tk.Button(window_search_ticket,text='搜索',command=show_ticket)
        confirm_search_ticket.place(x=90,y=100)
    ...
    bt_search=tk.Button(window_ticket_buy,text='搜索客票信息',command=SearchTicket)
    bt_search.place(x=180,y=300)
    
    look_profile=tk.Button(window_ticket_buy,text='查看个人信息',command=Profile)
    look_profile.place(x=180,y=350)
...
def Profile():
    ...
    def LookupAllStoppingStations():     
        ...
        record_of_ticket = session.query(tickets_info.go_time).filter(tickets_info.line_id==usr_lookup_line_id).first()
        
        station_1 = record_of_line.line_station_1
        station_2 = record_of_line.line_station_2
        station_3 = record_of_line.line_station_3
        station_4 = record_of_line.line_station_4
        station_5 = record_of_line.line_station_5
        
        gotime = record_of_ticket.go_time
        ...
        tk.Label(root11,text="您的起点是"+station_1+",经过"+station_2+","+station_3+","+station_4+",终点站是"+station_5+"\n"
                 +"列车将于"+gotime+"时刻出发，请提前做好准备。").pack()
