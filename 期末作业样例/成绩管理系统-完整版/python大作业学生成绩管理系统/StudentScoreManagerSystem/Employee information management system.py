...
class StartPage:
    def __init__(self, parent_window):
        ...
        self.window.geometry('600x470')  # 这里的乘是小x

        label = Label(self.window, text="员工信息管理系统", font=("Verdana", 20))
        label.pack(pady=90)  # pady=100 界面的长度
        ...
        Button(self.window, text="员工登陆", font=tkFont.Font(size=16), command=lambda: StudentPage(self.window), width=30,
               height=1, fg='white', bg='green', activebackground='black', activeforeground='white').pack()
        #Button(self.window, text="关于此系统", font=tkFont.Font(size=16), command=lambda: AboutPage(self.window), width=30,
        #       height=1,
         #      fg='white', bg='green', activebackground='black', activeforeground='white').pack()
        Button(self.window, text='退出系统', height=1, font=tkFont.Font(size=16), width=30, command=self.window.destroy,
               fg='white', bg='green', activebackground='black', activeforeground='white').pack()
        Label(self.window, text='基于Python+MySQL', font=('Verdana', 18)).pack(pady=30)
        Label(self.window, text='GUI界面', font=('Verdana', 18)).pack(pady=5)
...
class AdminPage:
    ...
    def login(self):
        ...
        # 数据库操作 查询管理员表
        db = pymysql.connect(host="localhost", port=3306, user="root", password="root", database="employee")  # 打开数据库连接
...
class StudentPage:
    def __init__(self, parent_window):
        ...
        self.window = tk.Tk()  # 初始框的声明
        self.window.title('员工登陆')
        self.window.geometry('300x450')  # 这里的乘是小x

        label = tk.Label(self.window, text='员工登陆', bg='green', font=('Verdana', 20), width=30, height=2)
        label.pack()

        Label(self.window, text='工号：', font=tkFont.Font(size=14)).pack(pady=25)
    ...
    def login(self):
        ...
        db = pymysql.connect(host="localhost", port=3306, user="root", password="root", database="employee")  # 打开数据库连接
        cursor = db.cursor()  # 使用cursor()方法获取操作游标
        sql = "SELECT * FROM employee_login_k WHERE employee_id = '%s'" % (self.student_id.get())  # SQL 查询语句
        try:
            # 执行SQL语句
            cursor.execute(sql)
            # 获取所有记录列表
            ...
            for row in results:
                ...
                stu_pass = row[1]
                # 打印结果
                print("employee_id=%s,employee_pass=%s" % (stu_id, stu_pass))
        except:
            print("Error: unable to fecth data")
            messagebox.showinfo('警告！', '用户名或密码不正确！')
        db.close()  # 关闭数据库连接
...
class AdminManage:
    def __init__(self, parent_window):
        ...
        # 表格的标题
        self.tree.column("工号", width=150, anchor='center')  # 表示列,不显示
        self.tree.column("姓名", width=150, anchor='center')
        self.tree.column("性别", width=100, anchor='center')
        self.tree.column("工作车间", width=100, anchor='center')
        ...
        self.age = []
        # 打开数据库连接
        db = pymysql.connect(host="localhost", port=3306, user="root", password="root", database="employee")
        cursor = db.cursor()  # 使用cursor()方法获取操作游标
        sql = "SELECT * FROM employee_k"  # SQL 查询语句
        try:
            # 执行SQL语句
            cursor.execute(sql)
            # 获取所有记录列表
        ...
        # 定义顶部区域
        # 定义左上方区域
        self.top_title = Label(self.frame_left_top, text="员工信息:", font=('Verdana', 20))
        ...
        # 年龄
        self.right_top_gender_label = Label(self.frame_left_top, text="工作车间：", font=('Verdana', 15))
        ...
        self.right_top_button1 = ttk.Button(self.frame_right_top, text='新建员工信息', width=20, command=self.new_row)
        self.right_top_button2 = ttk.Button(self.frame_right_top, text='更新选中员工信息', width=20,
                                            command=self.updata_row)
        self.right_top_button3 = ttk.Button(self.frame_right_top, text='删除选中员工信息', width=20,
    ...
    def new_row(self):
        ...
        if str(self.var_id.get()) in self.id:
            ...
            if self.var_id.get() != '' and self.var_name.get() != '' and self.var_gender.get() != '' and self.var_age.get() != '':
                ...
                messagebox.showinfo('警告！', '请填写员工数据')
    ...
    def updata_row(self):
        ...
        if res == True:
            if self.var_id.get() == self.row_info[0]:  # 如果所填学号 与 所选学号一致
                ...
                db = pymysql.connect(host="localhost", port=3306, user="root", password="root", database="employee")
                cursor = db.cursor()  # 使用cursor()方法获取操作游标
                sql = "UPDATE student_k SET name = '%s', job = '%s', age = '%s' \
				 WHERE id = '%s'" % (
                ...
                messagebox.showinfo('警告！', '不能修改员工工号！')
    ...
    def del_row(self):
        ...
        if res == True:
            ...
            db = pymysql.connect(host="localhost", port=3306, user="root", password="root", database="employee")
            cursor = db.cursor()  # 使用cursor()方法获取操作游标
            sql = "DELETE FROM employee_k WHERE id = '%s'" % (self.row_info[0])  # SQL 插入语句
            try:
                cursor.execute(sql)  # 执行sql语句
                db.commit()  # 提交到数据库执行
...
# 学生查看信息界面
class StudentView:
    def __init__(self, parent_window, employee_id):
        parent_window.destroy()  # 销毁主界面
        ...
        label = tk.Label(self.window, text='员工信息查看', bg='green', font=('Verdana', 20), width=30, height=2)
        label.pack(pady=20)
        ...
        db = pymysql.connect(host="localhost", port=3306, user="root", password="root", database="employee")
        cursor = db.cursor()  # 使用cursor()方法获取操作游标
        sql = "SELECT * FROM employee_k WHERE id = '%s'" % (employee_id)  # SQL 查询语句
        try:
            # 执行SQL语句
            cursor.execute(sql)
            # 获取所有记录列表
...
#
 #       self.window = tk.Tk()  # 初始框的声明
  #      self.window.title('关于')
   #     self.window.geometry('300x450')  # 这里的乘是小x
#
 #       label = tk.Label(self.window, text='员工信息管理系统', bg='green', font=('Verdana', 20), width=30, height=2)
  #      label.pack()
#       Label(self.window, text='python+MySQL', font=('Verdana', 18)).pack(pady=30)
 #       Label(self.window, text='GUI界面', font=('Verdana', 18)).pack(pady=5)
#
...
if __name__ == '__main__':
    try:
        ...
        # 如果数据表不存在则创建表 若存在则跳过
        # 设置主键唯一
        sql = """CREATE TABLE IF NOT EXISTS employee_k(
				id char(20) NOT NULL,
				name char(20) default NULL,
				job char(5) default NULL,  
        ...
        cursor.execute(sql)
        # 如果数据表不存在则创建表 若存在则跳过
        sql = """CREATE TABLE IF NOT EXISTS employee_login_k(
